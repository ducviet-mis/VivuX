...
function TheoryContent() {
  ...
  if (!validGrade) {
    return (
      <div className="container max-w-5xl py-16 text-center">
        ...
        <div className="mx-auto mt-6 max-w-md text-left"><MobileGradePicker allSizes hrefForGrade={(selectedGrade) => `/theory?grade=${selectedGrade}`} /></div>
      </div>
    );
  }
  ...
  return (
    <div className="study-page">
      <StudyHeading eyebrow="Lý thuyết · Hiểu từ nền tảng" title={`Lý thuyết Toán lớp ${grade}`} description="Từng khái niệm rõ ràng. Từng ví dụ dễ theo dõi. Học theo nhịp của bạn." />
      <div className="mb-5 md:hidden"><MobileGradePicker currentGrade={grade} hrefForGrade={(selectedGrade) => `/theory?grade=${selectedGrade}`} /></div>
      ...
      {error ? <div className="rounded-2xl border border-destructive/30 bg-destructive-soft p-5 text-destructive">{error}</div> : lessons.length === 0 ? (
        ...
        <div className="flex flex-col items-start gap-6 md:flex-row md:gap-8">
          <aside className="w-full shrink-0 md:sticky md:top-24 md:w-[240px]">
            <div className="mb-4 hidden items-center gap-2 font-semibold text-foreground md:flex"><BookOpen className="h-5 w-5 text-primary" />Nội dung học</div>
            <div className="flex snap-x gap-2 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:flex-col md:overflow-visible md:pb-0">
              {chapters.map((chapter) => {
                ...
                return (
                  <button key={chapter} type="button" aria-pressed={active} onClick={() => setActiveChapter(chapter)} className="study-chapter-button flex shrink-0 snap-start items-center justify-between gap-2 rounded-lg border px-4 py-3 text-left text-sm font-medium transition-colors hover:bg-muted md:w-full">
                    <span className="whitespace-nowrap leading-6 md:whitespace-normal">{chapter}</span>
          ...
          <main className="min-w-0 flex-1">
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              {visibleLessons.map((lesson, index) => (
                <Card key={lesson.id} className="study-lesson-card group rounded-xl transition-colors">
                  <CardContent className="flex h-full flex-col p-5 sm:p-6">
                    <div className="mb-4 flex items-start justify-between gap-4">
                      <div className="flex items-baseline gap-2 text-primary"><span className="text-3xl font-light tabular-nums tracking-tight">{String(index + 1).padStart(2, '0')}</span><span className="text-xs font-medium text-muted-foreground">Bài học</span></div>
                    ...
                    <div className="mt-5 grid grid-cols-1 gap-2 sm:grid-cols-2">
                      <Button asChild className="min-h-11"><Link href={'/theory/' + lesson.id}><BookOpen className="h-4 w-4" />Đọc lý thuyết</Link></Button>
                      {lesson.questionCount > 0 ? <Button asChild variant="outline" className="min-h-11"><Link href={'/theory/' + lesson.id + '/quiz'}><BookCheck className="h-4 w-4" />Kiểm tra</Link></Button> : <Button disabled variant="outline" className="min-h-11"><BookCheck className="h-4 w-4" />Chưa có câu hỏi</Button>}
                    </div>
                  </CardContent>
                </Card>
              ))}
