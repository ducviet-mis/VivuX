...
export function ResultPanel({ result, examConfig }: ResultPanelProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <Card className="border-t-4 border-t-primary shadow-card">
              ...
              {examConfig.answerKeys.map(key => {
                ...
                return (
                  <div key={key.questionNumber} className="grid grid-cols-12 gap-2 p-3 items-center hover:bg-muted/30">
                    ...
                    <div className="col-span-2 flex justify-center">
                      {isFlagged && <Flag className="w-4 h-4 text-destructive fill-current" />}
                    </div>
                  </div>
                );
              })}
          ...
          <CardContent>
            ...
            <div className="flex flex-wrap gap-2">
              {result.flaggedQuestions.map(qNum => (
                <div key={qNum} className="w-10 h-10 rounded-full bg-background border-2 border-destructive flex items-center justify-center font-bold text-destructive">
                  {qNum}
                </div>
              ))}
            </div>
