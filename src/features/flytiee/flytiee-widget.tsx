                  ...
                  {shopCategory === 'set' ? <>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      {FLYTIEE_SETS.map((item) => {
                        ...
                        return <button key={item.id} type="button" aria-pressed={selected} onClick={() => setSelectedSetId(item.id)} className={cn('min-h-[280px] overflow-hidden border p-3 text-left', styles.itemCard, selected ? 'border-primary bg-primary-soft' : 'border-border bg-card hover:border-primary/50', item.exclusiveMilestone && styles.phoenixCard)}>
                          <span className={cn('relative flex h-40 w-full items-center justify-center overflow-hidden rounded-lg', item.exclusiveMilestone ? styles.phoenixPreview : ACCESSORY_TONES[item.tone])}><FlytieeBird mood="idle" profile={{ ...flytiee.profile, equippedSetId: item.id, equipped: {} }} className="h-48 w-full" /><span className={cn('absolute right-2 top-2 inline-flex items-center gap-1 rounded-full px-2 py-1 text-[11px] font-bold shadow-soft', item.exclusiveMilestone ? styles.phoenixBadge : 'bg-card/90 text-special')}><Ticket aria-hidden="true" className="h-3.5 w-3.5" />{item.exclusiveMilestone ? `${item.exclusiveMilestone} ngày` : 'Sự kiện'}</span></span>
