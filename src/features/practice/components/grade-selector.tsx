  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
      {grades.map((grade) => (
        <Card
          key={grade.id}
          className="cursor-pointer hover:-translate-y-1 hover:shadow-card transition-all duration-200 border-l-4 border-l-teal-500 rounded-2xl bg-card"
          onClick={() => router.push(`/practice?grade=${grade.id}`)}
        >
          <CardContent className="p-6 text-center">
            <div className="text-4xl font-bold text-info mb-2">{grade.id}</div>
