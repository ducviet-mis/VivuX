...
import { DEFAULT_FLYTIEE_PROFILE, FLYTIEE_SETS } from '@/features/flytiee/config';
import { FlytieeBird } from '@/features/flytiee/flytiee-bird';
...
const MILESTONES = [
  ...
  { day: 200, reward: 'Set độc quyền Phượng Hoàng Bình Minh — lửa sống động trên chỏm đầu, cánh và đuôi' },
] as const;
...
export function StreakCard() {
  ...
  const unclaimedCount = ready && !error
    ? MILESTONES.filter((item) => item.day <= currentStreak && !claimedMilestones.includes(item.day)).length
    : 0;
  ...
  const claim = async (milestone: number) => {
    ...
    try {
      ...
      if ([10, 30, 50, 100, 200].includes(milestone)) window.dispatchEvent(new Event('flytiee:streak-reward'));
          ...
          <div className={styles.dialogBody}>
            ...
            <div className="space-y-3">
              {MILESTONES.map((item) => {
                ...
                return (
                  <article key={item.day} className={styles.milestone} data-day={item.day} data-state={claimed ? 'claimed' : reached ? 'ready' : 'locked'}>
                    <StreakFlame days={item.day} small />
                    <div className="min-w-0 flex-1">
                      <div className="flex flex-wrap items-center justify-between gap-2"><h3 className="font-extrabold text-foreground">Mốc {item.day} ngày</h3><span className="text-xs font-bold text-muted-foreground">{claimed ? 'Đã nhận' : reached ? 'Đã mở khóa' : `Còn ${item.day - currentStreak} ngày`}</span></div>
                      <p className="mt-1 text-sm leading-5 text-muted-foreground">{item.reward}</p>
                      {item.day === 200 && <div className={styles.legendaryPreview} aria-hidden="true"><FlytieeBird mood="idle" profile={{ ...DEFAULT_FLYTIEE_PROFILE, equippedSetId: 'phoenix-dawn' }} /></div>}
                      ...
                      {item.day === 100 && reached && !claimed && <label className="mt-3 block text-sm font-semibold text-foreground">Chọn Set yêu thích<select className={`mt-1 ${styles.select}`} value={selectedSetId} onChange={(event) => setSelectedSetId(event.target.value)}>{FLYTIEE_SETS.filter((set) => !set.exclusiveMilestone && set.milestone100Eligible !== false).map((set) => <option key={set.id} value={set.id}>{set.name}</option>)}</select></label>}
                      {reached && !claimed && <Button type="button" size="sm" className={`mt-3 ${styles.rewardButton}`} disabled={claiming !== null || Boolean(error)} onClick={() => void claim(item.day)}><Gift aria-hidden="true" className="h-4 w-4" />{claiming === item.day ? 'Đang nhận…' : 'Nhận quà'}</Button>}
                    </div>
                    {claimed ? <Check aria-hidden="true" className="mt-1 h-5 w-5 shrink-0 text-success" /> : !reached ? <LockKeyhole aria-hidden="true" className="mt-1 h-4 w-4 shrink-0 text-muted-foreground" /> : null}
                  </article>
                );
              })}
            </div>
